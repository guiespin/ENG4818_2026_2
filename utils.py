...
def EOS_PengRobinson(p, T, z, Tc, Pc, w, kij):
    ...
    mask = w <= 0.49
    if mask.any():
        m[mask] = 0.37464 + 1.54226*w - 0.26992*(w**2)
    if (~mask).any():
        m[~mask] = 0.379642 + 1.48503*w - 0.164423*(w**2) + 0.016666*(w**3)
    '''
    Tr = T/Tc
...
def Phi_PengRobinson(T, p, z, Tc, Pc, w, kij, fase = 'V'):
    ...
    if fase == 'L':
        ...
        Z = np.max(Z)
    ln_phi = (b_i/b)*(Z - 1) - np.log(Z - B) - (A/B)*(2*np.sum(z*np.sqrt(a_i[:, None]*a_i[None, :])*(1 - kij), axis = 1)/a - b_i/b)*np.log((Z + (1 + np.sqrt(2))*B)/(Z + (1 - np.sqrt(2))*B))
    return ln_phi
...
def Flash_Loop_TP(T, p, z, Tc, Pc, w, kij, FP, tol = 1e-5, maxit = 100):
    ...
    for _ in range(maxit):
        ...
        if np.isclose(fvap, 1., rtol = 0, atol = 1e-4):
            return 1., np.zeros_like(z), z
        if np.isclose(fvap, 0., rtol = 0, atol = 1e-4):
            return 0., z, np.zeros_like(z)
        ln_phi_L = Phi_FP(T, p, x, Tc, Pc, w, kij, fase = 'L')
        ln_phi_V = Phi_FP(T, p, y, Tc, Pc, w, kij, fase = 'V')
        ln_K = ln_phi_L - ln_phi_V
